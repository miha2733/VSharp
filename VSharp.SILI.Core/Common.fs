...
// ------------------------------- Type casting -------------------------------
    ...
    let rec typeIsType mtd leftType rightType = // left is subtype of right
        ...
        | ArrayType _ , ArrayType(_, SymbolicDimension name) ->
            if name.v = "System.Array" then makeTrue mtd else boolConst leftType rightType
        | ArrayType(_, SymbolicDimension _) as t1, (ArrayType _ as t2)  when t1 <> t2 ->
            boolConst leftType rightType
    ...
    let typeIsRef mtd typ ref =
        let typeCheck ref =
            ...
            let refType = baseTypeOfRef ref
            match typ with
            | InterfaceType _ when not <| Types.isObject refType -> makeFalse mtd
    ...
    let internal isValueType metadata termType =
        ...
        match termType with
        | TypeVariable(Id t) when t.IsValueType -> makeTrue metadata
...
// ------------------------------- Common functions for terms -------------------------------

    let commonPredicateCheck predicate term = Merging.commonGuardedErroredApply predicate (always true) term (List.forall snd)

// ---------------------------------------- Branching ---------------------------------------

    let commonStatelessConditionalExecution conditionInvocation thenBranch elseBranch merge merge2 errorHandler k =
    ...
    let statelessConditionalExecution conditionInvocation thenBranch elseBranch k = commonStatelessConditionalExecution conditionInvocation thenBranch elseBranch Merging.merge Merging.merge2Terms id k
