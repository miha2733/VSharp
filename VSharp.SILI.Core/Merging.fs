...
module internal Merging =
    ...
    and keysResolver<'a, 'b, 'c when 'c : equality> (r:bool) (read : bool -> 'a -> 'b -> termType -> term) keyMapper (getter:int -> 'a) (k : 'c memoryCell) hgvs =
        let key = keyMapper k
    ...
    and private blockMerge v gvs =
        ...
        let mergedBlock = updateFields v mergedFields
        [(Propositional.disjunction Metadata.empty gs, mergedBlock)]

    and private arrayMerge v gvs =
        let mtd = v.metadata
        let gs, vs = List.unzip gvs
    ...
    and private typedMerge = function
        | BoolMerge -> commonTypedMerge (fun _ gvs -> boolMerge gvs)
        | StructMerge
        | ClassMerge -> commonTypedMerge blockMerge
    ...
    and propagateGuard g v =
        ...
        | Block contents ->
            ...
            (Terms.True, updateFields v contents')
