...
type termNode =
        ...
        let arrayOfIndicesToString = function
            | Array(d, _, _, [(_, instantiator)], contents, _) ->
                let printed =
                    ...
                    | LazyInstantiator t -> sprintf "LI(%s): " (toString t)
                match d.term with
    ...
    override x.ToString() =
        ...
        let rec toStr parentPriority parentChecked indent term =
            ...
            | Struct(fields, t) ->
                fieldsToString indent fields |> formatIfNotEmpty indent |> sprintf "STRUCT %O[%s]" t
            | Class fields ->
            ...
            | Array(_, _, _, instantiators, contents, dimensions) ->
                ...
                let printedOne =
                    ...
                    | [_, i] ->
                        match i with
                        | DefaultInstantiator _ -> ""
                        | LazyInstantiator t -> sprintf "%s: " (toString t)
                    | _ -> sprintf "%s: " printed
...
and 'key heap when 'key : equality = commonHeap<'key, term, fql, termType>
and termHeap = term heap
and 'key memoryCell when 'key : equality = commonMemoryCell<'key, fql, termType>
...
// --------------------------------------- Primitives ---------------------------------------
    ...
    let (|Block|_|) = function
        | Struct(fields, _)
        | Class fields -> Some(Block fields)
        | _ -> None
    ...
    let updateBlock block newFields updateType =
        match block.term with
        | Struct(_, typ) -> Struct block.metadata newFields (updateType typ)
        | Class _ -> Class block.metadata newFields
        | term -> internalfailf "struct or class expected, %O recieved" term

    let updateFields block newFields = updateBlock block newFields id
    ...
    let private getTypeMap getType gvs =
    ...
    let commonTypeOf getType term =
        ...
        | Union gvs -> getTypeMap getType gvs
        | _ -> getType term
    ...
    let isBool t =      isPrimitiveTerm t && (typeOf >> Types.isBool) t
    let isInteger t =   isPrimitiveTerm t &&  (typeOf >> Types.isInteger) t
    let isReal t =      isPrimitiveTerm t && (typeOf >> Types.isReal) t
    let isNumeric t =   isPrimitiveTerm t && (typeOf >> Types.isNumeric) t
    let isString t =    isPrimitiveTerm t && (typeOf >> Types.isString) t
    let isFunction t =  isPrimitiveTerm t && (typeOf >> Types.isFunction) t
    let isPrimitive t = isPrimitiveTerm t && (typeOf >> Types.isPrimitive) t
