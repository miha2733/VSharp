...
module internal Arrays =
    ...
    let simplifyArraysEquality mtd x y indecesEq eq =
        ...
        let simplifyGInstantiatorEquality gInstor1 gInstor2 =
            let instorEq mtd x y =
                match x, y with
                | DefaultInstantiator typ1, DefaultInstantiator typ2
                | LazyInstantiator typ1, LazyInstantiator typ2
                | DefaultInstantiator typ1, LazyInstantiator typ2
                | LazyInstantiator typ1, DefaultInstantiator typ2 -> makeBool mtd (typ1 = typ2)
    ...
    let equalsArrayIndices mtd addr1 addr2 =
        ...
        | _, _ when isNumeric addr1 && isNumeric addr2 -> fastNumericCompare mtd addr1 addr2
    ...
    let (|Index|_|) = function
        ...
        | VectorT(ConcreteT(length, _), [_, DefaultInstantiator _], contents)
            when length :?> int = 1 -> Some(contents.[makeIndex Metadata.empty 0])
        | _ -> None
