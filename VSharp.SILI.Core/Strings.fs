...
module internal Strings =
    ...
    let makeLengthFQL fql = addToOptionFQL fql <| StructField(strLength, lengthType)
    ...
    let makeStringArray metadata length instor contents arrayFQL =
        ...
        let indexLengthKey = makePathKey arrayFQL (mkArrayIndex Char) indexLength lengthType
