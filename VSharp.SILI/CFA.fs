...
//            let k1 results = results |> List.unzip |> snd |> k
    ...
    module cfaBuilder =
        ...
        let private executeInstructions (stepInterpreter : ILInterpreter) cfg (cilState : cilState) : InsufficientInformationException option * cilState list =
            assert (cilState.ip.CanBeExpanded())
            stepInterpreter.ExecuteInstructions cfg cilState
