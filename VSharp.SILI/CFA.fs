...
//        try
//            let cfa = CFA.cfaBuilder.computeCFA funcId
//            x.ForwardExploration cfa funcId
//        with
//        | :? InsufficientInformationException -> base.Invoke funcId state k
    ...
    override x.EvaluateOneStep (funcId, cilState : cilState) =
        try
            ...
            let vertices = cfa.body.vertices.Values |> Seq.filter (fun (v : CFA.Vertex) ->
                v.Ip = ip && concreteValuesOnOperationalStack = List.filter IsIdempotent v.OpStack && v.OutgoingEdges.Count > 0) |> List.ofSeq
            match vertices with
