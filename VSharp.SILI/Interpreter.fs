...
// ------------------------------- Literals interning -------------------------------
    ...
    and internLiterals state ast =
        let rec internLiteralsRec literals (ast : INode) k =
            ...
            let interned = DecompilerServices.getPropertyOfNode ast "InternedLiterals" empty :?> string list
            if not <| List.isEmpty interned
            then k (List.append literals interned)
            else
                let literals =
                    match ast with
                    | :? ILiteralExpression as ast when MetadataTypes.fromMetadataType state ast.Value.Type = Types.String ->
                            ast.Value.Value.ToString() :: literals
                    | _ -> literals
                Cps.Seq.foldlk internLiteralsRec literals ast.Children k
    ...
    and reduceDecompiledMethod caller state this parameters (ast : IDecompiledMethod) initializerInvoke k =
        ...
        if metadataMethod.IsInternalCall || metadataMethod.IsPInvokeImpl then
            ...
            if externalImplementations.ContainsKey(fullMethodName) then
                ...
                internalCall fullMethodName argsAndThis state k)
...
// ------------------------------- Memory access -------------------------------
    ...
    and reduceLiteralExpression state (ast : ILiteralExpression) k =
        ...
        match mType with
        | Types.StringType -> obj.ToString() |> Memory.IsInternedLiteral state |> k
...
// ------------------------------- Objects construction -------------------------------
    ...
    and reduceArrayCreationExpression state (ast : IArrayCreationExpression) k =
        ...
        | Concrete(null, _) -> k <| Memory.MakeAndAllocateDefaultArray state dimensions typ
        | _ -> k <| Memory.MakeAndAllocateInitializedArray state dimensions (int ast.ArrayType.Rank) typ initializer))
    ...
    and reduceBaseOrThisConstuctorCall caller state this parameters qualifiedTypeName metadataMethod assemblyPath decompiledMethod k =
        let rec mutateFields this names types values initializers state =
            ...
            | name::names, typ::types, value::values, initializer::initializers ->
                ...
                | _ ->
                    let state = internLiterals state initializer
    ...
    and reduceObjectCreation returnRef (caller : locationBinding) state constructedType objectInitializerList collectionInitializerList (constructorSpecification : MethodSpecification) invokeArguments k =
        ...
        let reference, state =
            if isReferenceType
            then Memory.MakeAndAllocateDefaultStruct state constructedTermType
            else
