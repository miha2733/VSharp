...
module internal State =
    ...
    type internal state = { stack : stack; heap : heap; statics : staticMemory; iPool : internPool; frames : frames; pc : pathCondition }
...
// ------------------------------- Primitives -------------------------------
    ...
    let internal readPoolLocation (s : state) key = s.iPool.[key] |> fst3
...
// ------------------------------- Memory level -------------------------------
    ...
    let internal dumpMemory (s : state) =
        ...
        let mh = Heap.dump s.statics staticKeyToString in // TODO: ToString for internPool
...
// ------------------------------- Merging -------------------------------
    ...
    let internal merge guards states resolve : state =
        ...
        { stack = mergedStack; heap = mergedHeap; statics = mergedStatics; frames = frames; pc = path; iPool = mergedPool }
