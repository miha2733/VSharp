...
// ------------------------------- Primitives -------------------------------
    ...
    let rec internal makeSymbolicInstance metadata time source name = function
        | PointerType t ->
            let source' =
        ...
        | String ->
            let array = makeSymbolicInstance metadata time source (IdGenerator.newId()) <| ArrayType (Numeric typedefof<char>, ConcreteDimension 1)
            let fields = Heap.ofSeq (seq [ MakeStringKey "System.String.m_StringLength", (MakeNumber 0 metadata, time, time);
...
// ------------------------------- Referencing -------------------------------
    ...
    let internal referenceArrayLowerBound metadata arrayRef index =
        let newIndices = { index with metadata = Metadata.clone index.metadata} in
    ...
    let internal referenceArrayLength metadata arrayRef index =
        let newIndices = { index with metadata = Metadata.clone index.metadata} in
...
// ------------------------------- Comparison -------------------------------
    ...
    let private compareHeaps h1 h2 =
        ...
        Heap.cmpHeaps h1 h2 (List.filter (fun (k, v)-> (fst3 h1.[k]) <> v))
